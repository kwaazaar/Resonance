...
namespace Resonance.Tests.Consuming
{
    ...
    public class FunctionalOrderingTests
    {
        ...
        public void SerialDelivery()
        {
            ...
            _publisher.Publish(topicName, payload: "1", functionalKey: "f1", publicationDateUtc: publishedDateUtcBaseLine.AddSeconds(1)).Wait();
            _publisher.Publish(topicName, payload: "2", functionalKey: "f1", publicationDateUtc: publishedDateUtcBaseLine.AddSeconds(2)).Wait();
            _publisher.Publish(topicName, payload: "3", functionalKey: "f2", publicationDateUtc: publishedDateUtcBaseLine.AddSeconds(3)).Wait();
            _publisher.Publish(topicName, payload: "4", functionalKey: "f1", publicationDateUtc: publishedDateUtcBaseLine.AddSeconds(4)).Wait();
            _publisher.Publish(topicName, payload: "5", functionalKey: "f2", publicationDateUtc: publishedDateUtcBaseLine.AddSeconds(5)).Wait();

            var visibilityTimeout = 5;
            var p1 = _consumer.ConsumeNext(subName, visibilityTimeout: visibilityTimeout).Result.SingleOrDefault(); // p1 stands for payload "1"
            var p3 = _consumer.ConsumeNext(subName, visibilityTimeout: visibilityTimeout).Result.SingleOrDefault();
            var p2 = _consumer.ConsumeNext(subName, visibilityTimeout: visibilityTimeout).Result.SingleOrDefault();
            ...
            _consumer.MarkConsumed(p1.Id, p1.DeliveryKey).Wait(); // p1 should be gone, so p2 can be delivered
            p2 = _consumer.ConsumeNext(subName, visibilityTimeout: visibilityTimeout).Result.SingleOrDefault();
            var p4 = _consumer.ConsumeNext(subName, visibilityTimeout: visibilityTimeout).Result.SingleOrDefault();
            ...
            p2 = _consumer.ConsumeNext(subName, visibilityTimeout: visibilityTimeout).Result.SingleOrDefault();
            p3 = _consumer.ConsumeNext(subName, visibilityTimeout: visibilityTimeout).Result.SingleOrDefault();
            p4 = _consumer.ConsumeNext(subName, visibilityTimeout: visibilityTimeout).Result.SingleOrDefault();
            ...
            _consumer.MarkFailed(p2.Id, p2.DeliveryKey, Reason.Other("test")).Wait();
            p4 = _consumer.ConsumeNext(subName, visibilityTimeout: visibilityTimeout).Result.SingleOrDefault();
            Assert.Equal("4", p4.Payload);

            Thread.Sleep(TimeSpan.FromSeconds(visibilityTimeout + 1)); // Wait until visibilitytimeout of all items has expired
            p4 = _consumer.ConsumeNext(subName, visibilityTimeout: visibilityTimeout).Result.SingleOrDefault();
            var p5 = _consumer.ConsumeNext(subName, visibilityTimeout: visibilityTimeout).Result.SingleOrDefault();
