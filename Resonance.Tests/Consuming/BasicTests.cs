...
namespace Resonance.Tests.Consuming
{
    ...
    public class BasicTests
    {
        ...
        public void VisibilityTimeout()
        {
            ...
            ce2 = _consumer.ConsumeNext(subName).Result.SingleOrDefault();
        ...
        public void DeliveryDelay()
        {
            ...
            var ce1 = _consumer.ConsumeNext(subName).Result.SingleOrDefault();
        ...
        public void MaxDeliveries()
        {
            ...
            {
                var ce1 = _consumer.ConsumeNext(subName, visibilityTimeout: 1).Result.SingleOrDefault();
        ...
        public void TimeToLive()
        {
            ...
            _publisher.Publish(topicName).Wait();
            var ce = _consumer.ConsumeNext(subName, visibilityTimeout: 1).Result.SingleOrDefault();
            Assert.NotNull(ce);
            Thread.Sleep(TimeSpan.FromSeconds(1)); // Wait until visibility timeout has expired
            ce = _consumer.ConsumeNext(subName).Result.SingleOrDefault();
        ...
        public void TimeToLiveCapped()
        {
            ...
            _publisher.Publish(topicName, expirationDateUtc: DateTime.UtcNow.AddSeconds(1)).Wait();
            var ce = _consumer.ConsumeNext(subName, visibilityTimeout: 1).Result.SingleOrDefault();
            Assert.NotNull(ce);
            Thread.Sleep(TimeSpan.FromSeconds(1)); // Wait until visibility timeout has expired
            ce = _consumer.ConsumeNext(subName).Result.SingleOrDefault();
